...
ZY comment: for the distal segment, the force vector point to proximal; for the proximal segment, the force vector point to distal 
	...
	{														// This routine iterates for each muscle point
		...
		if ((segment_dist_point == segment_most_dist) && (segment_prox_point != segment_most_dist)) 
		{
			...
			for (int j=0; j<3; j++) 
			{
				...
				v1[j]=this->gblpnts[i+1][j];   //ZY: insertion
		...
		// intermediate segment
		//ZY comment: for biarticular muscle which attaches to the intermediate segment, firstly excutes the following statement
			...
			(segment_dist_point != segment_most_dist)) 
        {
			...
			for (int j=0; j<3; j++) 
			{
				v0[j]=this->gblpnts[i][j];    //ZY: origin
				v1[j]=this->gblpnts[i+1][j];  //ZY: via point
				v2[j]=segment_data[frame][(segment_data[frame][segment_most_prox]->dist)]->rot_centre[j];     //ZY: intermediate segment COR
				r[j]=v1[j]-v2[j];
				Fnorm[j]=v0[j]-v1[j];
			}
