...
void Segment::seg_solve_wrench_eqns(Vec_DP g) 
{
	...
	{
		v2[i]=-this->fd[i];
		v2[i+3]=-this->md_segmental_approach[i];         // Moment at distal end of segment in GCS
	...
	for (int j=0; j<3; j++) 
	{
		this->rhs_opt_approach_2[j]=W0[j]+vex0[j]+vex1[j];                   //ZY comment: W0 are acceleration and gravity component, vex0 and vex1 are external force component
		this->rhs_opt_approach_2[j+3]=W0[j+3]+v1[j+3]+W2[j+3]+W3[j+3];       //ZY commnet: W0 are acceleration and gravity component, V1 is inertia tensor component, 
                                                                             //ZY commnet: W2 and W3 are external force component
	}
