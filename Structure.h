...
public:
	...
	int markers;				// Number of static markers which would be keep on the segment during the dynamic process
	int calib_marker;           // marker used in the static process;
	int ex_forces;
	double marker_pos[10][3];		// Position of each marker on the segment for the given frame in the GCS (raw data transposed to GCS)
	double static_marker[10][3];    // calibration marker in the static process
	double marker_used_in_dynamic[10][3];
