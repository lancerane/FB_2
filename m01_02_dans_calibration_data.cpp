...
void m01_02_dans_calibration_data(Structure *calibrate_pos[], 
								  int segments, XML_Parameters* _parameters, Mat_DP &mean_calibration)
{

	m01_03_save_raw_positions(mean_calibration,calibrate_pos, _parameters);
	...
	*/
	...
	if ( _parameters->subject_sex == "female")
	{
		...
		// FEMALE SHANK
		calibrate_pos[1]->mass   = _parameters->subject_mass *0.0481;
		...
		double shank_saggital_ROG	  =  assumed_shank_length* 0.271; 
		double shank_longitudinal_ROG =  assumed_shank_length * 0.093; 
		double shank_transverse_ROG	  =  assumed_shank_length * 0.267; 
		...
		//FEMALE THIGH
		calibrate_pos[2]->mass   = _parameters->subject_mass *0.1478;
		...
		//FEMALE PELVIS
		calibrate_pos[3]->mass   = _parameters->subject_mass *0.1247;
		...
		calibrate_pos[4]->com[0] = calibrate_pos[3]->com[2] = 0;
	...
	else
	{
		...
		calibrate_pos[0]->com[0] = calibrate_pos[0]->com[2] = 0;
		double assumed_foot_length	 = _parameters->subject_height * 0.1482;
		...
		// MALE SHANK
		calibrate_pos[1]->mass   = _parameters->subject_mass *0.0433;
		calibrate_pos[1]->com[1] = 0.4459;
		calibrate_pos[1]->com[0] = calibrate_pos[0]->com[2] = 0;
		double assumed_shank_length	  = _parameters->subject_height * 0.2493;
		double shank_saggital_ROG	  = assumed_shank_length * 0.255; 
		double shank_longitudinal_ROG = assumed_shank_length* 0.103; 
		double shank_transverse_ROG	  = assumed_shank_length * 0.249; 
		calibrate_pos[1]->inertia[0]  =  shank_saggital_ROG * shank_saggital_ROG * calibrate_pos[1]->mass;
		calibrate_pos[1]->inertia[1]  =  shank_longitudinal_ROG * shank_longitudinal_ROG * calibrate_pos[1]->mass;
		calibrate_pos[1]->inertia[2]  =  shank_transverse_ROG * shank_transverse_ROG * calibrate_pos[1]->mass;

		//MALE THIGH
		calibrate_pos[2]->mass   = _parameters->subject_mass *0.1416;
		...
		//MALE PELVIS
		calibrate_pos[3]->mass   = _parameters->subject_mass *0.1117;
